...
class _PatientDispatchHomeTab extends StatefulWidget {
  final Patient patient;

  const _PatientDispatchHomeTab({required this.patient});

  @override
  State<_PatientDispatchHomeTab> createState() => _PatientDispatchHomeTabState();
}

class _PatientDispatchHomeTabState extends State<_PatientDispatchHomeTab> {
  // متغير للتحكم في ظهور التنبيهات
      ...
      builder: (context, app, child) {
        ...
        return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snapshot) {
            ...
            final livePatient = widget.patient.copyWith(
              assignedInstitutionId: liveData['assignedInstitutionId']?.toString(),
              assignedInstitutionCode: liveData['assignedInstitutionCode']?.toString(),
              assignedInstitutionName: liveData['assignedInstitutionName']?.toString(),
...
// باقي الكود (ServicesTab, Header, etc.) يبقى كما هو تماماً دون أي تغيير
...
class _PatientHeader extends StatelessWidget {
  ...
  const _PatientHeader({required this.patient, required this.isConnected, required this.deviceStatus});
...
class _RiskSummaryCard extends StatelessWidget {
  ...
  const _RiskSummaryCard({required this.assessment, required this.caseStatus});
...
class _QuickFlagsRow extends StatelessWidget {
  ...
  const _QuickFlagsRow({required this.arrhythmiaAbnormal, required this.respiratoryAbnormal, required this.alertsCount});
...
class _FlagCard extends StatelessWidget {
  ...
  final Color color;
  final IconData icon;
  const _FlagCard({required this.title, required this.value, required this.color, required this.icon});
...
class _ConnectionBadge extends StatelessWidget {
  ...
  final String text;
  const _ConnectionBadge({required this.connected, required this.text});
...
String _riskLabel(RiskLevel level) {
  switch (level) {
    ...
    case RiskLevel.attention: return 'Attention Needed';
...
Color _riskColor(RiskLevel level) {
  switch (level) {
    ...
    case RiskLevel.highRisk: return Colors.deepOrange;
    case RiskLevel.emergency: return Colors.red;
  }
}

String _prettyText(String value) {
  return value.replaceAll('_', ' ').split(' ').map((e) => e.isEmpty ? e : '${e[0].toUpperCase()}${e.substring(1)}').join(' ');
}
